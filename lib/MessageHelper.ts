import type { IHttp, IMessageBuilder, IModify, IModifyCreator, IPersistence, IRead, IRoomBuilder } from '@rocket.chat/apps-engine/definition/accessors';
...
import type { GetMessageResponse } from './MicrosoftGraphApi';
import { downloadOneDriveFileAsync, getMessageAttachments, MessageContentType } from './MicrosoftGraphApi';
...
export const sendRocketChatOneOnOneMessageAsync = async (message: string, sender: IUser, receiver: IUser, read: IRead, modify: IModify): Promise<string> => {
...
export const generateUploadCallback =
	...
	async (): Promise<{ rocketChat: string; teams: string }[]> => {
		const uploadIds: { rocketChat: string; teams: string }[] = [];
		if (attachments && attachments.length > 0) {
			if (uploadFiles) {
				const attachmentsToUpload = attachments.filter((a) => a.contentType === TeamsAttachmentType.File);
				(
					await Promise.all(
...
export const mapTeamsMessageToRocketChatMessage = async ({
	...
	if (messageContentType && messageContentType === MessageContentType.Html) {
		...
		text = await buildRocketChatMessageText({
			nodes: isBridged ? extractMainTextNodesFromBridgedMessageNodes(parsedNodes) : parsedNodes,
			attachments,
			read,
		});
	}
...
export const getBridgedMessageFormatV2 = (originalSenderName: string, message: string): string => {
	return (
		...
		`</p>` +
		// Message paragraph
		`<p style="font-style:inherit; font-weight:inherit; margin-bottom:0; margin-left:0; margin-right:0; margin-top:0">${message}</p>` +
		// Closing blockquote
		`</blockquote>`
	);
};
...
export const mapRocketChatMessageToTeamsMessageV2 = async ({
	...
	if (forceBridgedMessage) {
		teamsMessage = getBridgedMessageFormatV2(originalSenderName || 'Rocket.Chat User', teamsMessage);
	}
...
export const buildExtraInfoFileName = (originalFilename: string, data: any): string => {
	// Encode metadata as base64
	const encoded = Buffer.from(JSON.stringify({ extraInfo: data }), 'utf8').toString('base64');
...
export const combineRocketChatMessagesToTeamsMessage = async ({
	...
	const targetMessages = messages.filter((message) => message.id && !deletedMessages?.has(message.id));
	...
	if (uploadMappings) {
		targetUploadMappings = uploadMappings.filter((uploadMap) => !deletedUploads?.has(uploadMap.rocketchatUploadId));
	} else {
		targetUploadMappings = (await UploadMapping.findByTeamsMessageId(read, messageIdMapping.teamsMessageId)).filter(
	...
	for (const att of allAttachments) {
		if (att?.id && !uniqueAttachmentsMap.has(att.id)) {
	...
	return {
		...
		shouldDeleteTeamsMessage: targetMessages.length === 0 && teamsAttachmentIds.length === 0,

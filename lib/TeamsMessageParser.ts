...
import type { Attachment } from './MicrosoftGraphApi';
...
export async function buildRocketChatMessageText({
	...
	async function parseNode(node: Node, extra?: Record<string, any>): Promise<string> {
		...
		if (node.type === 'element') {
			switch (node.tagName) {
				case 'a':
					const href = node.attributes.find((attr) => attr.key === 'href')?.value;
					...
					return (
						await Promise.all(
							node.children.map((c) =>
								parseNode(c, {
									ol: true,
									index: 'tagName' in c && c.tagName === 'li' ? index++ : undefined,
								}),
							),
						)
					).join('');
				case 'li':
					...
					if (extra?.ol) {
						return `${extra?.index ? extra.index : 1}. ${(await Promise.all(node.children.map((c) => parseNode(c)))).join('')}\n`;
					}
					return `- ${(await Promise.all(node.children.map((c) => parseNode(c)))).join('')}\n`;
				...
				case 'attachment':
					const id = node.attributes.find((attr) => attr.key === 'id')?.value;
					if (!id) {
						return ``;
					}
					...
					switch (attachment?.contentType) {
						case TeamsAttachmentType.MessageReference:
							let msgUrl = '';
							const mapping = await MessageMapping.findByTeamsMessageId(read, attachment.id);
	...
	const results = await Promise.all(nodes.map((node) => parseNode(node)));
...
export const extractMainTextNodesFromBridgedMessageNodes = (nodes: ParseResult): ParseResult => {

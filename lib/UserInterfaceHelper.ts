...
import {
    getTeamsChatMembersAsync,
    searchTeamsUsersAsync,
} from "./MicrosoftGraphApi";
import type {
    TeamsChatMember,
    GetTeamsChatMembersResult,
} from "./MicrosoftGraphApi";
...
export const decodeButtonState = (value: string): LoadMoreButtonState =>
    ...
    ) as LoadMoreButtonState;

const encodeViewMembersButtonState = (state: ViewMembersButtonState): string =>
    Buffer.from(JSON.stringify(state)).toString("base64");

export const decodeViewMembersButtonState = (
    value: string,
): ViewMembersButtonState =>
    JSON.parse(
        Buffer.from(value, "base64").toString("utf-8"),
    ) as ViewMembersButtonState;

export const encodeUserOptionValue = (
    id: string,
    displayName: string,
): string => Buffer.from(JSON.stringify([id, displayName])).toString("base64");

export const decodeUserOptionValue = (
    value: string,
...
export const openAddTeamsUserContextualBarBlocksAsync = async (
    ...
    const view = createContextualBarBlocks(
        modify,
        loadedUsers,
        currentRoom.id,
        nextLink,
    ...
    await modify
        .getUiController()
        .openSurfaceView(view, { triggerId }, operator);
};
...
export const updateAddTeamsUserContextualBarAsync = async (options: {
    ...
    const { actionId, value, roomId, read, http, persistence, app, modify } =
        options;
    ...
    if (actionId === UIActionId.TeamsUserSearchInput) {
        ...
        return createContextualBarBlocks(
            modify,
            loadedUsers,
            roomId,
            nextLink,
...
export const createContextualBarBlocks = (
    ...
    blocks.addInputBlock({
        ...
        element: blocks.newPlainTextInputElement({
            ...
            dispatchActionConfig: [
                InputElementDispatchAction.ON_CHARACTER_ENTERED,
            ],
        }),
    ...
    blocks.addInputBlock({
        ...
        label: blocks.newPlainTextObject(
            UIElementText.TeamsUserNameSearchTitle,
        ),
    });
...
export const updateViewTeamsMembersContextualBarAsync = async (options: {
    ...
    const {
        nextLink: pageUrl,
        loadedMembers: prevMembers,
        threadId,
    } = decodeViewMembersButtonState(value);
    ...
    const mergedMembers: TeamsChatMember[] = [
        ...prevMembers,
        ...result.members,
    ];
...
export const openViewTeamsMembersContextualBarAsync = async (
    ...
    const result = await getTeamsChatMembersAsync(
        http,
...
const createViewMembersContextualBarBlocks = async ({
    ...
    if (members.length === 0) {
        blocks.addSectionBlock({
            text: blocks.newMarkdownTextObject(
                UIElementText.ViewMembersEmptyText,
            ),
        });
    } else {
        ...
        for (const chunk of memberChunks) {
            const chunkPromises = chunk.map(async (member) => {
                const userMapping = await UserMapping.findByTeamsUserId(
                    read,
                    member.userId,
                );

                if (userMapping) {
                    const rcUser = await read
                        .getUserReader()
                        .getById(userMapping.rocketChatUserId);
                    if (rcUser?.username) {
    ...
    return {
        ...
        title: blocks.newPlainTextObject(
            UIElementText.ViewMembersContextualBarTitle,
        ),
        type: UIKitSurfaceType.CONTEXTUAL_BAR,
        blocks: blocks.getBlocks(),
    };

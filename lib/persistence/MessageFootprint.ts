...
export const MessageFootprint = {
    generate(message: IMessage, room: IRoom, sender: IUser): string {
        ...
        const combinedProperties =
            ...
            `${room.id}${room.type}${room.displayName}` +
    ...
    async getExistenceInfo(
        message: IMessage,
        read: IRead,
    ): Promise<{ itDoesMessageFootprintExists: boolean; messageFootprint: string } | undefined> {
        try {

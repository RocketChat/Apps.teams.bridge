...
export const RecentActivity = {
	...
	async isRecent(options: { read: IRead; rcUserId: string; teamsThreadId: string; kind: ActivityKind; windowMs?: number }): Promise<boolean> {
		const { read, rcUserId, teamsThreadId, kind, windowMs = DEFAULT_WINDOW_MS } = options;
		const associations = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, `${rcUserId}:${teamsThreadId}:${kind}`),
		];
		...
		try {
			const results = (await read.getPersistenceReader().readByAssociations(associations)) as RecentActivityModel[];
	...
	async delete(options: { persistence: IPersistence; rcUserId: string; teamsThreadId: string; kind: ActivityKind }): Promise<void> {
		const { persistence, rcUserId, teamsThreadId, kind } = options;
		const associations = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, `${rcUserId}:${teamsThreadId}:${kind}`),
	...
	async deleteStale(read: IRead, persistence: IPersistence): Promise<number> {
		...
		try {
			...
			for (const r of results) {
				if (r.timestamp < cutoff && r.rcUserId && r.teamsThreadId && r.kind) {
					await persistence.removeByAssociations([
						new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
						new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, `${r.rcUserId}:${r.teamsThreadId}:${r.kind}`),
					]);
					count++;
				}
			}
		} catch (error) {
			console.error('[MS Teams RecentActivity] Failed during stale cleanup:', error);
		}

...
export const OAuthNonce = {
	async persist(persis: IPersistence, rocketChatUserId: string, nonce: string): Promise<void> {
	...
	async findAndDelete(read: IRead, persis: IPersistence, rocketChatUserId: string): Promise<string | null> {
		const associations: Array<RocketChatAssociationRecord> = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.USER, rocketChatUserId),
		];
	...
	async deleteStale(read: IRead, persis: IPersistence): Promise<void> {

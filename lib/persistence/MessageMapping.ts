...
export const MessageMapping = {
	...
	async delete(options: { persistence: IPersistence; rocketChatMessageId: string; teamsMessageId: string }): Promise<void> {
		...
		await Promise.all([persistence.removeByAssociations(byRC), persistence.removeByAssociations(byTeams)]);
	},
};

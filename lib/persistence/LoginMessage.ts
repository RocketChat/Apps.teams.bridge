...
export const LoginMessage = {
	async save(options: { persistence: IPersistence; rocketChatUserId: string; wasSent: boolean }): Promise<void> {
	...
	async get(options: { read: IRead; rocketChatUserId: string }): Promise<boolean> {
	...
	async delete(persistence: IPersistence, rocketChatUserId: string): Promise<void> {
		const associations: Array<RocketChatAssociationRecord> = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.USER, rocketChatUserId),
		];

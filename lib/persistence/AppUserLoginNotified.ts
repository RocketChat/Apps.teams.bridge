...
export const AppUserLoginNotified = {
	async set(persistence: IPersistence, roomId: string): Promise<void> {
		...
		await persistence.updateByAssociations(associations, { notified: true, date: new Date() }, true);
	},

	async isSet(persistenceRead: IPersistenceRead, roomId: string): Promise<boolean> {
		const associations: Array<RocketChatAssociationRecord> = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.ROOM, roomId),
		];
	...
	async isSetToday(persistenceRead: IPersistenceRead, roomId: string): Promise<boolean> {
		const associations: Array<RocketChatAssociationRecord> = [
			new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, KEY),
			new RocketChatAssociationRecord(RocketChatAssociationModel.ROOM, roomId),
		];

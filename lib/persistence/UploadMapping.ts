...
export const UploadMapping = {
	...
	async delete(options: { persistence: IPersistence; rocketchatUploadId: string; teamsMessageId: string }): Promise<void> {
		...
		await Promise.all([persistence.removeByAssociations(byUploadId), persistence.removeByAssociations(byTeamsMessageId)]);
	},
};

import type { IHttp, IRead } from '@rocket.chat/apps-engine/definition/accessors';
...
export type Inlines = Markup | Timestamp | InlineCode | Image | Link | UserMention | Emoji | Color | InlineKaTeX;
...
export const createTeamsHTMLMessage = (root: Root, siteUrl?: string) => {
	...
	const render = (node: AnyNode): string => {
		...
		switch (n.type) {
			...
			case 'UNORDERED_LIST': {
				return `<ul>${n.value.map((li) => `<li>${li.value.map(render).join('')}</li>`).join('')}</ul>`;
			}
			case 'ORDERED_LIST': {
				return `<ol>${n.value
					.map((li) => {
						const liNode = li;
						const numberAttr = liNode.number != null ? ` value="${liNode.number}"` : '';
						return `<li${numberAttr}>${liNode.value.map(render).join('')}</li>`;
					})
					.join('')}</ol>`;
			}
			case 'TASKS': {
				...
				return `<ul>${n.value
					.map((t) => {
						...
						return `<li>${checkbox} ${task.value.map(render).join('')}</li>`;
					})
					.join('')}</ul>`;
			}
			case 'QUOTE': {
			...
			case 'CODE': {
				const code = n.value.map((cl) => cl.value.value).join('\n');
				const lang = n.language ? ` data-lang="${esc(n.language)}" class="language-${esc(n.language)}"` : '';
			...
			case 'LINK': {
				...
				const label = Array.isArray(n.value.label) ? n.value.label.map(render).join('') : render(n.value.label as any);
				return `<a href=\"${href}\" title=\"${label}\" target=\"_blank\" rel=\"noreferrer noopener\">${label}</a>`;
			}
			case 'IMAGE': {
				const src = esc(n.value.src.value);
				const alt = esc(stripHtml(Array.isArray(n.value.label) ? n.value.label.map(render).join('') : render(n.value.label as any)));
			...
			case 'TIMESTAMP': {
				...
				return `<time data-timestamp="${esc(n.value.timestamp)}" data-format="${n.value.format}">${esc(n.value.timestamp)}</time>`;
			}
...
export const attachAttachments = ({ html, attachmentIds = [] }: { html: string; attachmentIds?: string[] }) => {
 ...
 */
export const attachMessageReferences = async (read: IRead, http: IHttp, html: string, accessToken: string) => {
	if (!html) {
	...
	for (const rcMsgId of referencedIds) {
		try {
			...
			if (mapping?.teamsMessageId) {
				...
				transformed = transformed.replace(new RegExp(`<msg id="${rcMsgId}"><\\/msg>`, 'g'), attachmentTag);
			...
			if (msg?.id && msg.room) {
				try {
					...
					transformed = transformed.replace(new RegExp(`<msg id="${rcMsgId}"><\\/msg>`, 'g'), link);
					continue;
				} catch {
			...
			transformed = transformed.replace(new RegExp(`<msg id="${rcMsgId}"><\\/msg>`, 'g'), '[referenced message not available]');
		} catch (err) {

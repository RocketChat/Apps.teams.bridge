...
export const handlePreRoomUserLeaveAsync = async (options: {
	...
	if (leavingRocketChatUserId === appUser?.id) {
	...
	const embeddedLoginUser = await UserMapping.findByRCUserId(read, leavingRocketChatUserId);
	...
	const threadMembers = await listMembersInChatThreadAsync(http, roomRecord.teamsThreadId, accessToken);
	...
	if (leavingMember) {
		...
		await removeMemberFromChatThreadAsync(http, roomRecord.teamsThreadId, leavingMember.id, accessToken);
	}
};

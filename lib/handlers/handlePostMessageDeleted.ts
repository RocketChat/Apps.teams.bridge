...
import { deleteTextMessageInChatThreadAsync, updateTextMessageInChatThreadAsync } from '../MicrosoftGraphApi';
...
export const handlePostMessageDeletedAsync = async (options: {
	...
	if (await PreventRegistry.capture(persistence, `PreventPostMessageDeleteHook/${message.id}`)) {
	...
	const { messageIdMapping, uploadMappings, currentUploadMapping, mainMessage } = await resolveMappings(read, { ...message, id: msgId });

	if (!messageIdMapping && !currentUploadMapping && uploadMappings.length === 0) {
		return;
	}
	...
	const teamsIds = {
		messageId: messageIdMapping?.teamsMessageId || currentUploadMapping?.teamsMessageId,
		threadId: messageIdMapping?.teamsThreadId || currentUploadMapping?.teamsThreadId,
	};
	...
	const { text, shouldDeleteTeamsMessage, attachments } = await combineRocketChatMessagesToTeamsMessage({
		read,
	...
	if (shouldDeleteTeamsMessage) {
		await PreventRegistry.set(persistence, `PreventPostMessageDeleteHook/${message.id}`);
		await deleteTextMessageInChatThreadAsync(http, senderUser.teamsUserId, teamsIds.messageId, teamsIds.threadId, accessToken);
	} else {
...
async function resolveMappings(read: IRead, message: IMessage & { id: string }) {
	...
	let messageIdMapping = await MessageMapping.findByRCMessageId(read, message.id);

	let uploadMappings: UploadMappingModel[] = [];
	if (message.file?._id) {
		uploadMappings = await UploadMapping.findAllByRCUploadId(read, message.file._id);
	}

	const currentUploadMapping = uploadMappings.find((u) => u.rocketchatUploadId === message.file?._id);

	if (currentUploadMapping && !messageIdMapping) {
		messageIdMapping = await MessageMapping.findByTeamsMessageId(read, currentUploadMapping.teamsMessageId);
		if (messageIdMapping) {
			mainMessage = (await read.getMessageReader().getById(messageIdMapping.rocketChatMessageId)) || null;
		}
	} else if (!currentUploadMapping && messageIdMapping) {
		uploadMappings = await UploadMapping.findByTeamsMessageId(read, messageIdMapping.teamsMessageId);
		mainMessage = message;
	}

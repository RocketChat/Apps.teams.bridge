import { IRead, IHttp, IPersistence } from "@rocket.chat/apps-engine/definition/accessors";
...
export const handleWebhookSecretCreationAsync = async ({
    ...
    try {
        ...
        if (!webhookSecret) {
            app.getLogger().info(
                "Webhook secret is not created. Creating it now.",
            );
            ...
            app.getLogger().info(
                "Webhook secret created and subscriptions were renewed.",
            );
        }
    } catch (error) {
        app.getLogger().error(

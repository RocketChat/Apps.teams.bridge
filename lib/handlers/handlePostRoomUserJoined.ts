...
export const handlePostRoomUserJoinedAsync = async (options: {
	...
	if (joiningUser.id !== appUser.id) {
		...
		if (!appUserToken) {
			app.getLogger().warn(
				`[TeamsBridge] No app user access token available to add Teams member ${embeddedLoginUser.teamsUserId} to thread ${roomRecord.teamsThreadId}.`,
			);
			return;
		}
		...
		await addMemberToChatThreadAsync(http, roomRecord.teamsThreadId, embeddedLoginUser.teamsUserId, appUserToken);
	...
	app.getLogger().info(
		`[TeamsBridge] Room "${room.displayName || room.id}" is now an active bridge room ` + `(app user added by ${context.inviter?.username ?? 'unknown'}).`,
	);
	...
	if (!appUserToken) {
		...
		await notifyRocketChatUserInRoomAsync(AppUserAddedToRoomMessageText, appUser, inviter, room, modify.getNotifier());
	}
};

...
export const handleAddTeamsUserContextualBarSubmitAsync = async (options: {
	...
	const { app, http, modify, operator, persistence, read, room, teamsUserIdsToSave } = options;
	...
	for (const encodedValue of teamsUserIdsToSave) {
		...
		const result = await addMemberToChatThreadAsync(http, roomRecord.teamsThreadId, teamsUserId, accessToken);
	...
	if (addedNames.length > 0) {
		...
		const text =
			addedNames.length === 1 ? `${nameList} has been added to this channel on MS Teams.` : `${nameList} have been added to this channel on MS Teams.`;

		const msg = modify.getCreator().startMessage().setSender(appUser).setRoom(room).setText(text);
	...
	if (failedNames.length > 0) {
		...
		const text =
			failedNames.length === 1 ? `Failed to add ${nameList} to this channel on MS Teams.` : `Failed to add ${nameList} to this channel on MS Teams.`;
		await notifyRocketChatUserInRoomAsync(text, appUser, operator, room, read.getNotifier());
	}
};

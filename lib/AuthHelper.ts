...
export const getAccessTokenForRegistration = async (options: {
	...
	const { registration, http, persistence, app, forceRefresh = false } = options;
	...
	if (!registration?.expires || epochInSecond > registration.expires || forceRefresh) {
		if (registration?.refreshToken) {
			...
			try {
				const response = await renewUserAccessTokenAsync(http, registration.refreshToken, aadTenantId, aadClientId, aadClientSecret, userType);
				...
				app.getLogger().error(`Failed to renew access token for user ${registration.rocketChatUserId}`, error);
			}
		}
...
export const getAppAccessTokenAsync = async (options: { read: IRead; persistence: IPersistence; http: IHttp; app: TeamsBridgeApp }): Promise<string | null> => {
	...
	try {
		const response = await http.post(`https://login.microsoftonline.com/${aadTenantId}/oauth2/v2.0/token`, {

...
import { AuthenticationEndpointPath, LoginAppUserMessageText, SubscriberEndpointPath } from '../lib/Const';
...
export class ResubscribeMessages implements ISlashCommand {
	...
	public i18nDescription: string = 'teamsbridge-resubscribe-messages_command_description';
	...
	public async executor(context: SlashCommandContext, read: IRead, modify: IModify, http: IHttp, persis: IPersistence): Promise<void> {
		...
		if (!commandSender.roles.includes('admin')) {
			...
			await notifyRocketChatUserInRoomAsync(message, commandSender, commandSender, room, modify.getNotifier());
			return;
		}
		...
		if (!userAccessToken) {
			const loginUrl = await getLoginUrlAsync(persis, aadTenantId, aadClientId, authEndpointUrl, appUser.id, 'bot');
		...
		try {
			...
			const userMapping = await UserMapping.findByRCUserId(read, appUser.id);
			if (!userMapping) {
				throw new Error('User not found or the teams user is not synced with Rocket.Chat');
			}
			...
			await notifyRocketChatUserInRoomAsync(message, appUser, commandSender, room, modify.getNotifier());
		} catch (error) {
			this.app.getLogger().error(`Failed to subscribe to messages`, error);

import type { IApp } from '@rocket.chat/apps-engine/definition/IApp';
...
import type { IApiResponseJSON } from '@rocket.chat/apps-engine/definition/api/IResponse';
...
export class AuthenticationEndpoint extends ApiEndpoint {
	...
	public async get(request: IApiRequest, endpoint: IApiEndpointInfo, read: IRead, modify: IModify, http: IHttp, persis: IPersistence): Promise<IApiResponse> {
		...
		const persistResult = await this.persistAndSubscribe(read, persis, http, rocketChatUserId, type, tokenResult);
		if (!persistResult) {
			return this.errorResponse();
		}
	...
	private async validateStateAndNonce(
		...
		try {
			const parsed = JSON.parse(Buffer.from(request.query.state, 'base64').toString('utf-8')) as Record<string, unknown>;
			if (typeof parsed.rc_uid !== 'string' || parsed.rc_uid.length === 0 || (parsed.type !== 'bot' && parsed.type !== 'normal')) {
				this.app.getLogger().warn('Authentication rejected — malformed state parameter.');
				return null;
			}

			const rocketChatUserId = parsed.rc_uid;
			...
			return { rocketChatUserId, type, accessCode: request.query.code };
		} catch (error) {
			this.app.getLogger().error(`Authentication — state validation failed: ${(error as Error)?.message ?? String(error)}`);
			return null;
		}
	}

	private async fetchAppEnvironment(
		read: IRead,
	): Promise<{ aadTenantId: string; aadClientId: string; aadClientSecret: string; authEndpointUrl: string } | null> {
		try {
			...
			this.app.getLogger().error(`Authentication — settings fetch failed: ${(error as Error)?.message ?? String(error)}`);
			return null;
		}
	}
	...
	private async exchangeAndValidateTokens(
		...
		try {
			const response = await getUserAccessTokenAsync(http, accessCode, authEndpointUrl, aadTenantId, aadClientId, aadClientSecret, type);
			...
			if (existingMapping !== null && existingMapping.rocketChatUserId !== rocketChatUserId) {
				if (existingMapping.rocketChatUserId === appUser?.id) {
				...
				if (!existingUser?.isEnabled) {
					...
					return {
						conflictError: `This Teams account is already linked to another Rocket.Chat user (@${existingUser.username}). Please log out from Teams on the other account first.`,
					};
				}
			}
			...
			this.app.getLogger().error(`Authentication — token exchange/profile failed: ${(error as Error)?.message ?? String(error)}`);
			return null;
		}
	}

	private async persistAndSubscribe(read: IRead, persis: IPersistence, http: IHttp, rocketChatUserId: string, type: 'bot' | 'normal', tokenData: any) {
		try {
			const { userAccessToken, refreshToken, expiresIn, extExpiresIn, teamsUserId, appUser } = tokenData;

			await Promise.all([
				UserRegistration.persist(persis, rocketChatUserId, userAccessToken, refreshToken, expiresIn, extExpiresIn),
			...
			if (rocketChatUserId === appUser?.id) {
			...
			this.app.getLogger().error(`Authentication — persistence/subscription failed: ${(error as Error)?.message ?? String(error)}`);
			return false;
		}
	}
